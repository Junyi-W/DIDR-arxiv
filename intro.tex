...
% \begin{wrapfigure}{tr}{0.98\linewidth}
% \centering
% \vspace{-0.5in}
% \includegraphics[width=0.95\linewidth]{pictures/samples.jpg}
% \caption{$1024\times 1024$ images from one-step SDXL aligned via \textsc{Didr}. Prompts listed in Appendix~\ref{app:prompts}.}
% \label{fig:teaser}
% \vspace{-0.3in}
% \end{wrapfigure}
...
\begin{itemize}[leftmargin=6pt, noitemsep, topsep=2pt]
    % \item We propose \textsc{Didr}, a principled trajectory-level alignment framework for one-step generators that formulates preference optimization as minimization over reward-tilted diffusion marginals---providing the unified trajectory-level objective that prior endpoint-only methods lacked.

    % \item We derive the Diffused Reward Score (DRS) and Diffused Reward Proxy (DRP) as a tractable realization of this objective, propagating clean-image rewards across the full diffusion trajectory using only a frozen reference model.

    % \item Without image training data, we demonstrate on one-step SDXL at $1024\times1024$ that \textsc{Didr} consistently improves preference metrics over existing one-step baselines, achieves the best PickScore--FID trade-off among one-step alignment methods, and generalizes to a 6B DiT-based Z-Image backbone.
...
% \centering
% \includegraphics[width=0.95\linewidth]{pictures/samples.jpg}
% \caption{$1024\times 1024$ images from one-step SDXL aligned via \textsc{Didr}. Prompts are listed in Appendix~\ref{app:prompts}.}
% \label{fig:teaser}
% \end{figure}
